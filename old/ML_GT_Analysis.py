...
def get_input_variables(input_variables, GT_angle_max):
    ...
    event_alt = input_variables[1]*event_alt_max
    GT_angle = np.pi/2 - input_variables[2]*GT_angle_max*np.pi/180
    stage_mass_ratios = np.array([0.8,0.2]) 
    accel_init = input_variables[3:5]*2
    stage_delta_vee_ratios = np.array([0.5,0.5])#np.array([input_variables[5],input_variables[6]])/(input_variables[5] + input_variables[6])
    alpha = alpha_max*input_variables[7]
    
    return m_dry, event_alt, GT_angle, stage_mass_ratios, stage_delta_vee_ratios, alpha, accel_init

def calculate_result(pop, weights, m_dry_max, event_alt_max, alpha_max, GT_angle_max, factor, refine):
...
def run_last(pop, weights, m_dry_max, event_alt_max, alpha_max, GT_angle_max):
...
def run(W_vel, W_alt, W_angle, perc_elite, perc_lucky, perc_mutation, mutation_chance, pop_size, generations, n_inputs, samples, m_dry_max, event_alt_max, factor, refine, alpha_max, GT_angle_max):
...
#    plt.close("all")
    ...
    for KK in range(samples):
        ...
        for I in range(generations):
            ...
            pop = calculate_result(pop, weights, m_dry_max, event_alt_max, alpha_max, GT_angle_max, factor,refine)
...
def n_d_runfile(W_vel, W_alt, W_angle, perc_elite = 1, perc_lucky = 1, perc_mutation = 1, perc_selected = 1, mutation_chance = 1, samples = 1, generations = 1, m_dry_max = 1, event_alt_max = 1, factor = 1, refine = False, alpha_max = 0, GT_angle_max = 1):
    ...
    gen_count_stats, pop, best_performance = run(W_vel, W_alt, W_angle, perc_elite, perc_lucky, perc_mutation, mutation_chance, pop_size, generations, n_inputs, samples, m_dry_max, event_alt_max, factor, refine, alpha_max, GT_angle_max)
    ...
    v, phi, r, theta, m, t, ind, grav_delta_vee = run_last(pop['actions'][0], np.array([0,0,0]), m_dry_max, event_alt_max, alpha_max, GT_angle_max)
...
#            start_time = time.time()
#            generation_stats, pop, v, phi, r, theta, m, t, ind, grav_delta_vee = n_d_runfile(W_vel, W_alt, W_angle, perc_elite, perc_lucky, perc_mutation, perc_selected, mutation_chance, samples, generations, m_dry_max, event_alt_max, factor, False, alpha_max, GT_angle_max)
...
#        factor = 0.1
#        if already_run == False:
#            generation_stats, pop, v, phi, r, theta, m, t, ind, grav_delta_vee = n_d_runfile(W_vel, W_alt, W_angle, perc_elite, perc_lucky, perc_mutation, perc_selected, mutation_chance, samples, generations, m_dry_max, event_alt_max, factor, True, alpha_max, GT_angle_max)
#            pop_best = pop['actions'][0]
#            
...
#            ready_for_refine2 = True
    ...
    v, phi, r, theta, m, t, ind, grav_delta_vee = run_last(pop_best, np.array([0,0,0]), m_dry_max, event_alt_max, alpha_max, GT_angle_max)
    ind_burn = ind + 21000 - 4