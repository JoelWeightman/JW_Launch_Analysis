...
def model_burn(variables,t,G_c,M_e,R_e,m_dot,thrust_design,rocket_diam,Cd,R_air,R_prop,gamma,P_chamber,T_chamber,alpha):
    ...
    T = thrust_current(altitude,m_dot,P_chamber,T_chamber,gamma,R_air,R_prop,g)
...
def set_variables(m_dry, stage_mass_ratios):
    ...
    thrust_design = 30000
    Isp_design = m_dot_design(thrust_design,15e3,P_chamber,T_chamber,gamma,R_air,R_prop,g)[1]
    ...
    return g, G_c, M_e, R_e, t_steps, Isp_design, thrust_design, rocket_diam, Cd, target_altitude, delta_vee_req, stage_m_dry, orbital_vel, R_air, R_prop, gamma, P_chamber, T_chamber

def calculate_trajectory(delta_vee_req, stage_delta_vee_ratios, Isp_design, stage_m_dry, t_steps, g, G_c, M_e, R_e, thrust_design, rocket_diam, Cd, event_alt, GT_angle, R_air, R_prop, gamma, P_chamber, T_chamber, alpha, accel_init, coast_on = False):
    
    Isp_design = np.zeros(2)
    
    Isp_design[0] = m_dot_design(thrust_design,5e3,P_chamber,T_chamber,gamma,R_air,R_prop,g)[1]
    Isp_design[1] = m_dot_design(thrust_design,30e3,P_chamber,T_chamber,gamma,R_air,R_prop,g)[1]
    ...
    ### First Stage
    thrust_design = (stage_m_init[0]*g)*(1+accel_init[0])
    stage_m_dot = np.zeros(2)
    t_burn = np.zeros(2)
    stage_m_dot[0], Isp = m_dot_design(thrust_design,0,P_chamber,T_chamber,gamma,R_air,R_prop,g)
    ...
    stage_m_dot[1], Isp = m_dot_design(thrust_design,altitude,P_chamber,T_chamber,gamma,R_air,R_prop,g)
    ...
    trajectory3 = odeint(model_burn, second_stage_conds, t3, args=(G_c,M_e,R_e,stage_m_dot[1],thrust_design,rocket_diam,Cd,R_air, R_prop,gamma,P_chamber,T_chamber,alpha))
...
def run_trajectory(m_dry,stage_mass_ratios,event_alt,GT_angle,stage_delta_vee_ratios,weights,alpha,accel_init):
    ...
    g, G_c, M_e, R_e, t_steps, Isp_design, thrust_design, rocket_diam, Cd, target_altitude, delta_vee_req, stage_m_dry, orbital_vel, R_air, R_prop, gamma, P_chamber, T_chamber = set_variables(m_dry, stage_mass_ratios)
    v, phi, r, theta, m, t, ind = calculate_trajectory(delta_vee_req, stage_delta_vee_ratios, Isp_design, stage_m_dry, t_steps, g, G_c, M_e, R_e, thrust_design, rocket_diam, Cd, event_alt, GT_angle, R_air, R_prop, gamma, P_chamber, T_chamber, alpha, accel_init)
...
def run_trajectory_final(m_dry,stage_mass_ratios,event_alt,GT_angle,stage_delta_vee_ratios,weights,alpha,accel_init):
    ...
    g, G_c, M_e, R_e, t_steps, Isp_design, thrust_design, rocket_diam, Cd, target_altitude, delta_vee_req, stage_m_dry, orbital_vel, R_air, R_prop, gamma, P_chamber, T_chamber = set_variables(m_dry, stage_mass_ratios)
    v, phi, r, theta, m, t, ind = calculate_trajectory(delta_vee_req, stage_delta_vee_ratios, Isp_design, stage_m_dry, t_steps, g, G_c, M_e, R_e, thrust_design, rocket_diam, Cd, event_alt, GT_angle, R_air, R_prop, gamma, P_chamber, T_chamber, alpha, accel_init, True)
    ...
    grav_delta_vee = np.sum(g*np.cos(phi[:-t_steps*20])*(t[1:-t_steps*20+1] - t[:-t_steps*20]))
